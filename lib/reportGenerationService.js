          ...
          try {
            ...
            holdingsWithData.forEach(h => {
              try {
                ...
                const fundName = h?.fundName || 'N/A';
                const currency = h?.currency || '-';
                const origAllocStr = isNaN(originalAlloc) ? '-' : originalAlloc.toFixed(2);
                const allocStr = isNaN(allocation) ? '0.00' : allocation.toFixed(2);
                const fundCapitalStr = formatCurrency(fundCapital);
                const fundValStr = formatCurrency(fundValuation);
                const fundPnLStr = fundPnL.toFixed(2);
                const fundPnLAmt = formatCurrency(fundValuation - fundCapital);

                holdingsRows += '<tr><td>' + fundName + '</td><td class="numeric">' + currency + '</td><td class="numeric">' + origAllocStr + '%</td><td class="numeric">' + allocStr + '%</td><td class="numeric">' + fundCapitalStr + '</td><td class="numeric">' + fundValStr + '</td><td class="numeric ' + fundPnLClass + '">' + fundPnLStr + '%</td><td class="numeric ' + fundPnLClass + '">' + fundPnLAmt + '</td></tr>';
              } catch (e) {
