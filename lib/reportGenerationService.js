...
export const generateHTMLReport = (reportData, holdingsSets) => {
  // Defensive: ensure all required data is provided
  if (!reportData) {
  ...
  // Ensure holdingsSets is an array
  ...
  // Only process accounts that have matching holdings
  const accountMetrics = (Array.isArray(accounts) ? accounts : [])
    .filter((account, idx) => idx < safeHoldingsSets.length)
    .map((account, idx) => {
      const perf = Array.isArray(performance) ? performance[idx] : null;
  ...
  try {
    ...
    <body>
      ...
      <div class="page">
        ...
        <div class="metrics-section">
          ...
          <div class="metric-card">
            ...
            <div class="metric-value">${safeHoldingsSets.length}</div>
      ...
      ${accountMetrics.map((metric, idx) => {
        ...
        const holdingsArray = holdings?.holdings || holdings || [];

        // Calculate the actual portfolio total from holdings (not from form input)
        const portfolioTotal = holdingsArray.reduce((sum, h) => sum + (parseFloat(h?.marketValueSgd) || 0), 0);

        const holdingsWithAllocation = (Array.isArray(holdingsArray) ? holdingsArray : []).map(h => ({
          ...h,
                ...
                ${!hasCapitalData ? `
                  ...
                  <thead>
                    <tr>
                      ...
                      <th class="numeric">Portfolio Value (SGD)</th>
                    </tr>
                  </thead>
                  <tbody>
                    ${(holdingsWithPnL || []).map(h => {
                      try {
                        ...
                        const valuation = parseFloat(h?.marketValueSgd || 0);
                        return `
                          <tr>
                            <td>${h?.fundName || 'N/A'}</td>
                            <td class="numeric">${isNaN(allocation) ? '0.00' : allocation.toFixed(2)}%</td>
                            <td class="numeric currency">${formatCurrency(valuation)}</td>
                          </tr>
                        `;
                      } catch (e) {
                        console.error('[generateHTMLReport] Error rendering holding:', h, e);
                        return '<tr><td colspan="3">Error rendering holding</td></tr>';
                      }
                    }).join('')}
                  </tbody>
                  <tr class="total-row">
                  ...
                  <tbody>
                    ${(holdingsWithPnL || []).map(h => {
                      try {
                        ...
                        <tr>
                          <td>${h?.fundName || 'N/A'}</td>
                          ${hasAllOriginalAllocation ? `<td class="numeric">${isNaN(originalAlloc) ? '-' : originalAlloc.toFixed(2)}%</td>` : ''}
                          <td class="numeric">${isNaN(allocation) ? '0.00' : allocation.toFixed(2)}%</td>
                          <td class="numeric currency">${formatCurrency(fundCapital)}</td>
                          <td class="numeric currency">${formatCurrency(fundValuation)}</td>
                          ${hasAllOriginalAllocation && h?.fundPnLPercent !== undefined && !isNaN(h.fundPnLPercent) ? `<td class="numeric ${h.fundPnLPercent >= 0 ? 'positive' : 'negative'}">${formatPercent(h.fundPnLPercent)}</td>` : ''}
                        </tr>
                      `;
                      } catch (e) {
                        console.error('[generateHTMLReport] Error rendering holding (full view):', h, e);
                        return '<tr><td colspan="5">Error rendering holding</td></tr>';
                      }
                    }).join('')}
                  </tbody>
                  <tr class="total-row">
  ...
  `;
    ...
    console.error('[generateHTMLReport] Error generating template:', err);
    console.error('[generateHTMLReport] accountMetrics:', accountMetrics);
    console.error('[generateHTMLReport] safeHoldingsSets:', safeHoldingsSets);
