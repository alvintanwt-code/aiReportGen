  ...
  accountsWithHoldings.forEach((account, idx) => {
    try {
      ...
      if (hasOriginalAllocation) {
        ...
        holdingsWithData.forEach(h => {
          try {
            ...
            const fundCapital = initialCapital > 0 ? (initialCapital * (originalAlloc || 0)) / 100 : 0;
            ...
            holdingsRows += '<tr><td>' + (h?.fundName || 'N/A') + '</td><td class="numeric">' + origAllocStr + '%</td><td class="numeric">' + allocStr + '%</td><td class="numeric">' + formatCurrency(fundCapital) + '</td><td class="numeric">' + formatCurrency(fundValuation) + '</td><td class="numeric ' + fundPnLClass + '">' + fundPnLStr + '%</td><td class="numeric ' + fundPnLClass + '">' + formatCurrency(fundValuation - fundCapital) + '</td></tr>';
          } catch (e) {
        ...
        // Minimal table: Fund Name, Weightage %, Valuation
        tableHeadersHTML = '<tr><th>Fund Name</th><th class="numeric">Weight %</th><th class="numeric">Valuation</th></tr>';
        totalRowColspan = '1';
        ...
        holdingsWithData.forEach(h => {
          try {
            ...
            holdingsRows += '<tr><td>' + (h?.fundName || 'N/A') + '</td><td class="numeric">' + allocStr + '%</td><td class="numeric">' + formatCurrency(fundValuation) + '</td></tr>';
          } catch (e) {
            console.error('[Holdings Row Error]:', h, e);
            holdingsRows += '<tr><td colspan="3">Error rendering holding</td></tr>';
          }
        });
      }
      ...
      const totalRowHTML = hasOriginalAllocation
        ? '<tr class="total-row"><td colspan="' + totalRowColspan + '">TOTAL</td><td class="numeric">' + formatCurrency(initialCapital) + '</td><td class="numeric">' + formatCurrency(currentValuation) + '</td><td class="numeric ' + pAndLClass + '">' + pAndLPercentStr + '%</td><td class="numeric ' + pAndLClass + '">' + formatCurrency(pAndL.pAndLAmount) + '</td></tr>'
        : '<tr class="total-row"><td colspan="' + totalRowColspan + '">TOTAL</td><td class="numeric">' + formatCurrency(currentValuation) + '</td></tr>';
