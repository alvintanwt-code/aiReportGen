  ...
  const html = `
    ...
    <body>
      ...
      ${accountMetrics.map((metric, idx) => {
        const { account, perf, holdings, initialCapital, currentValuation, pAndL, cagr } = metric;
        ...
        const holdingsWithAllocation = (holdings.holdings || []).map(h => ({
          ...h,
          allocation: totalCurrentValuation > 0 ? ((h.valuationInSgd || 0) / totalCurrentValuation) * 100 : 0,
          originalAllocation: h.originalAllocationPercent || null,
        }));

        // Check if all holdings have original allocation filled
        const hasAllOriginalAllocation = holdingsWithAllocation.every(h => h.originalAllocation !== null && h.originalAllocation !== undefined && h.originalAllocation !== '');

        // Calculate P&L per fund if original allocation is available
        const holdingsWithPnL = holdingsWithAllocation.map(h => {
          if (hasAllOriginalAllocation && h.originalAllocation) {
                ...
                <tbody>
                  ${holdingsWithPnL.map(h => `
                    <tr>
                      ...
                      ${hasAllOriginalAllocation ? `<td class="numeric">${h.originalAllocation ? h.originalAllocation.toFixed(2) : '-'}%</td>` : ''}
                      <td class="numeric">${h.allocation.toFixed(2)}%</td>
                      <td class="numeric currency">${formatCurrency((initialCapital * h.allocation) / 100)}</td>
                      <td class="numeric currency">${formatCurrency(h.valuationInSgd)}</td>
                      ${hasAllOriginalAllocation ? `<td class="numeric ${h.fundPnLPercent >= 0 ? 'positive' : 'negative'}">${h.fundPnLPercent ? formatPercent(h.fundPnLPercent) : '-'}</td>` : ''}
                    </tr>
                  `).join('')}
                </tbody>
                <tr class="total-row">
                  ...
                  ${hasAllOriginalAllocation ? `<td class="numeric ${pAndL.pAndLAmount >= 0 ? 'positive' : 'negative'}" style="padding: 15px;">${formatPercent(pAndL.pAndLPercent)}</td>` : ''}
                </tr>
              </table>
            </div>
