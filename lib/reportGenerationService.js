...
/**
 * Convert text to naming case (title case) and remove text in parentheses
 * e.g., "INFINITY US 500 STOCK (CLASS A)" -> "Infinity Us 500 Stock"
 */
const toNamingCase = (text) => {
  if (!text) return text;
  // Remove text within parentheses and trim extra spaces
...
export const generateHTMLReport = (reportData, holdingsSets) => {
  ...
  accountsWithHoldings.forEach((account, idx) => {
    try {
      ...
      // Prefer XIRR (dated cashflow-weighted) when available; fall back to CAGR
      // for older saved reports that don't have the field yet.
      const cagr = perf?.xirr || perf?.cagr || calculateCAGR(capitalInvested, currentValuation, perf?.inceptionDate);
