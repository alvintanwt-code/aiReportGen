              ...
              <table class="holdings-table">
                <thead>
                  <tr>
                    ...
                    <th class="numeric">Allocation %</th>
                    <th class="numeric">Valuation (SGD)</th>
                    <th class="numeric">P&L %</th>
                    <th class="numeric">P&L (SGD)</th>
                  </tr>
                </thead>
                <tbody>
                  ${holdingsWithAllocation.map(h => `
                    <tr>
                      ...
                      <td>${h.currency || '-'}</td>
                      <td class="numeric">${parseFloat(h.units || 0).toLocaleString('en-SG', { maximumFractionDigits: 2 })}</td>
                      <td class="numeric">${formatCurrency(h.unitPrice, h.currency)}</td>
                      <td class="numeric">${h.allocation.toFixed(2)}%</td>
                      <td class="numeric currency">${formatCurrency(h.valuationInSgd)}</td>
                      <td class="numeric ${h.gainLossPercent >= 0 ? 'positive' : 'negative'}">${formatPercent(h.gainLossPercent || 0)}</td>
                      <td class="numeric ${h.gainLossSgd >= 0 ? 'positive' : 'negative'}">${formatCurrency(h.gainLossSgd || 0)}</td>
                    </tr>
                  `).join('')}
                </tbody>
                <tr class="total-row">
                  <td colspan="5" style="padding: 15px;">TOTAL</td>
                  <td class="numeric currency" style="padding: 15px;">${formatCurrency(currentValuation)}</td>
                  <td class="numeric" style="padding: 15px;">-</td>
                  <td class="numeric ${pAndL.pAndLAmount >= 0 ? 'positive' : 'negative'}" style="padding: 15px;">${formatCurrency(pAndL.pAndLAmount)}</td>
                </tr>
              </table>
            </div>
