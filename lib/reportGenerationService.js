      ...
      if (hasOriginalAllocation) {
        ...
        holdingsWithData.forEach(h => {
          try {
            ...
            holdingsRows += '<tr><td>' + (h?.fundName || 'N/A') + '</td><td class="numeric">' + allocStr + '%</td><td class="numeric">' + formatCurrency(capitalInvested > 0 ? (capitalInvested / holdingsWithData.length) : 0) + '</td><td class="numeric">' + formatCurrency(fundValuation) + '</td></tr>';
          } catch (e) {
            console.error('[Holdings Row Error]:', h, e);
