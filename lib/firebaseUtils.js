...
export const loadFNASummary = async (userId, clientId) => {
  try {
    const path = `users/${userId}/fnaSummaries/${clientId}`;
    console.log('[loadFNASummary] Loading from path:', path);
    const summaryRef = doc(db, path);
    const summaryDoc = await getDoc(summaryRef);
    console.log('[loadFNASummary] Document exists:', summaryDoc.exists());
    if (summaryDoc.exists()) {
      console.log('[loadFNASummary] Found data:', Object.keys(summaryDoc.data()));
      return summaryDoc.data();
    }
    console.log('[loadFNASummary] Document does not exist');
    return null;
  } catch (error) {
    console.error('[loadFNASummary] Error:', error.message);
    return null;
  }
};
